...
def sinkhorn_loss_primal(x,y,epsilon,n,niter, device=torch.device('cuda'), dtype=torch.float):
	...
	mu = 1./n * torch.ones(n, device=device, dtype=dtype, requires_grad=False)
	nu = 1./n * torch.ones(n, device=device, dtype=dtype, requires_grad=False)
...
def sinkhorn_loss_dual(x,y,epsilon,n,niter, device=torch.device('cuda'), dtype=torch.float) :
